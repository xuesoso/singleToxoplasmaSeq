...
gpd = {x:y for x,y in zip(df_id['gene'].values,
                                      df_id['product'].values)}
data = sat.pp.convert_id_to_gene(sat.pp.load_dataframes(htseq_input,\
                                remove_last_nrows=5).T, df_id,
                                 start='transcript')
...
df_star.index = ['_'.join(x.split('_')[:4]) for x in df_star.index.values]
adata.var['product'] = sat.get_product(adata.var_names.values, gpd)
...
gpd = {x:y for x,y in zip(adata_raw.var.index.values, adata_raw.var['product'].values)};
